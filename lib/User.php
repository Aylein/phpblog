...
class User {
    ...
    public static function Add($user){
        ...
        if(count($obj[1]) != 1) return new Message(false, "no_create", "创建用户失败");
        return new Message(true, "create", "创建成功", new User($obj[1][0]));
    }
    ...
    public static function SignIn($pass){
        ...
        $en = new Entity();
        $res = $en->First($str, $paras);
        if(!$res) return new Message(false, "no_login", "登陆失败");
        return new Message(true, "get", "登陆成功", new User($res)); //new User($res);
    }
